...
float git_angle_ultrasonic(int angle){
  ...
  for(count = 0; count < 9;count++){
     distance_buf[count]  = Ultrasonic_measurement(Ultrasonic_TrigPin,Ultrasonic_EchoPin);delay(200);
...
void init_all(int *gyro_fd,int *laser_fd){
  ...
  *gyro_fd = com_gyro();                                                                                 // 初始化陀螺仪
   ...
   system("echo \"2\" >> /home/pi/data.txt ");
}
/*-------------------------------------------------------------------------*/
int main(void){
  /*--------------------------------------------  ------*/
